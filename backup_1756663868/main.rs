...
fn decrypt_bytes(state:&State<AppState>, s:&str) -> Result<Vec<u8>,String>{
  let st=s.trim_start();
  if st.starts_with('[') || (st.starts_with('{' ) if st.starts_with('[') || (st.starts_with('{')  &&  &&  !st.contains("\"nonce\"")) { return Ok(st.as_bytes().to_vec()) }if st.starts_with('[') || (st.starts_with('{')  &&  &&  !st.contains("\"nonce\"")) { return Ok(st.as_bytes().to_vec()) } !st.contains(""nonce"")) { return Ok(st.as_bytes().to_vec()) }
