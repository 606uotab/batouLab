...
fn decrypt_bytes(state: &State<AppState>, s: &str) -> Result<Vec<u8>, String> {
  let st = s.trim();
  // Si c'est du JSON "classique" (array) ou un objet JSON qui ne ressemble pas à notre enveloppe chiffrée,
  // on renvoie tel quel.
  if st.starts_with('[') || (st.starts_with('{') && !st.contains("\"nonce\"")) {
    return Ok(st.as_bytes().to_vec());
  }

  // Fallback: on tente de lire une enveloppe {"v":1,"nonce":"...","ct":"..."} et renvoyer le contenu décodé.
  // (NOTE: pas de déchiffrement ici, juste pour rétablir la compilation — on branchera la crypto réelle ensuite.)
  let v: serde_json::Value = serde_json::from_str(st).map_err(|e| e.to_string())?;
  let ct_b64 = v.get("ct").and_then(|x| x.as_str()).ok_or("missing ct")?;
  let data = base64::decode(ct_b64).map_err(|e| e.to_string())?;
  Ok(data)
}
if st.starts_with('[') || (st.starts_with('{') && !st.contains("\"nonce\"")) {
    return Ok(st.as_bytes().to_vec());
