...
fn decrypt_bytes(state: &State<AppState>, s: &str) -> Result<Vec<u8>, String> {
    ...
    // Si déjà JSON lisible (liste/objet sans nonce), renvoyer tel quel.
    if st.starts_with('[') || (st.starts_with('{') && !st.contains("\"nonce\"")) {
        return Ok(st.as_bytes().to_vec());
    }

    // Sinon, enveloppe chiffrée { nonce, ct }.
    ...
    let c = cipher_from_state(state)?;
    let pt = c.decrypt(XNonce::from_slice(&n), ct.as_ref()).map_err(|e| e.to_string())?;
    Ok(pt)
}

// -- commandes Tauri : AU NIVEAU MODULE (pas dans main) -----------------------
...
fn lock_history(state: tauri::State<AppState>) -> Result<(), String> {
    let mut slot = state.key.lock().unwrap();
    if let Some(mut k) = slot.take() { k.zeroize(); }
    Ok(())
}

#[tauri::command]
fn status_history(state: tauri::State<AppState>) -> Result<bool, String> {
...
fn save_history_enc(json: String, state: tauri::State<AppState>) -> Result<(), String> {
    ensure_dirs()?;
    let enc = encrypt_bytes(&state, json.as_bytes())?;
    std::fs::write(chats_root().join("index.json"), enc).map_err(|e| e.to_string())?;
    Ok(())
}
...
fn load_history_enc(state: tauri::State<AppState>) -> Result<String, String> {
    ensure_dirs()?;
    let p = chats_root().join("index.json");
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            unlock_history, lock_history, status_history,
            save_history_enc, load_history_enc
        ])
        // .plugin(tauri_plugin_fs::init()) // <- inutile si tu utilises std::fs ; sinon laisse-le si déjà en dépendance.
        .run(tauri::generate_context!())
        .expect("error while running tauri application");
}

// ----- Point 2: fichiers par session + index léger -----
use std::path::PathBuf;

fn session_path(id:&str) -> PathBuf { chats_root().join(format!("{}.json", id)) }

fn read_index_dec(state:&State<AppState>) -> Result<Vec<serde_json::Value>, String> {
  let p = chats_root().join("index.json");
  if !p.exists() { return Ok(vec![]) }
  let s = std::fs::read_to_string(&p).map_err(|e| e.to_string())?;
  let dec = decrypt_bytes(state, &s)?;
  let text = String::from_utf8(dec).map_err(|e| e.to_string())?;
  let v: serde_json::Value = serde_json::from_str(&text).map_err(|e| e.to_string())?;
  Ok(v.as_array().cloned().unwrap_or_default())
}
fn write_index_enc(state:&State<AppState>, arr:&[serde_json::Value]) -> Result<(), String> {
  std::fs::create_dir_all(chats_root()).map_err(|e| e.to_string())?;
  let plain = serde_json::to_string(arr).map_err(|e| e.to_string())?;
  let enc = encrypt_bytes(state, plain.as_bytes())?;
  std::fs::write(chats_root().join("index.json"), enc).map_err(|e| e.to_string())
}

#[tauri::command]
fn list_sessions(state:tauri::State<AppState>) -> Result<String,String> {
  let arr = read_index_dec(&state)?;
  Ok(serde_json::to_string(&arr).unwrap())
}

#[tauri::command]
fn save_session_enc(id:String, json:String, state:tauri::State<AppState>) -> Result<(),String> {
  std::fs::create_dir_all(chats_root()).map_err(|e| e.to_string())?;
  let enc = encrypt_bytes(&state, json.as_bytes())?;
  std::fs::write(session_path(&id), enc).map_err(|e| e.to_string())?;

  // Metadonnées pour index.json
  let v: serde_json::Value = serde_json::from_str(&json).map_err(|e| e.to_string())?;
  let meta = serde_json::json!({
    "id": v.get("id").cloned().unwrap_or(serde_json::json!(id)),
    "title": v.get("title").cloned().unwrap_or(serde_json::json!("Nouveau chat")),
    "provider": v.get("provider").cloned().unwrap_or(serde_json::json!("mistral")),
    "model": v.get("model").cloned().unwrap_or(serde_json::json!("mistral-small-latest")),
    "updatedAt": v.get("updatedAt").cloned().unwrap_or(serde_json::json!(0)),
  });

  let mut arr = read_index_dec(&state)?;
  // remplace ou ajoute
  if let Some(pos) = arr.iter().position(|x| x.get("id").and_then(|y| y.as_str()) == meta.get("id").and_then(|y| y.as_str())) {
    arr[pos] = meta;
  } else {
    arr.push(meta);
  }
  write_index_enc(&state, &arr)
}

#[tauri::command]
fn load_session_enc(id:String, state:tauri::State<AppState>) -> Result<String,String> {
  let p = session_path(&id);
  if !p.exists() { return Ok("".into()) }
  let s = std::fs::read_to_string(&p).map_err(|e| e.to_string())?;
  let dec = decrypt_bytes(&state, &s)?;
  Ok(String::from_utf8(dec).map_err(|e| e.to_string())?)
}

#[tauri::command]
fn delete_session(id:String, state:tauri::State<AppState>) -> Result<(),String> {
  let _ = std::fs::remove_file(session_path(&id));
  let mut arr = read_index_dec(&state)?;
  arr.retain(|x| x.get("id").and_then(|y| y.as_str()) != Some(id.as_str()));
  write_index_enc(&state, &arr)
}

#[tauri::command]
fn export_session_text(id:String, state:tauri::State<AppState>) -> Result<String,String> {
  let raw = load_session_enc(id, state)?;
  if raw.is_empty() { return Ok(String::new()) }
  let v: serde_json::Value = serde_json::from_str(&raw).map_err(|e| e.to_string())?;
  let title = v.get("title").and_then(|x| x.as_str()).unwrap_or("Conversation");
...
fn sessions_dir() -> std::path::PathBuf { chats_root().join("sessions") }
fn ensure_sessions_dir() -> Result<(), String> { std::fs::create_dir_all(sessions_dir()).map_err(|e| e.to_string()) }
