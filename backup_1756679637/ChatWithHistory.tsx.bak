...
import { type Conversation, listConversations, loadConversation, saveConversation, deleteConversation, exportConversationAsText } from "../storage/session";

type Msg = { role: "user" | "assistant" | "system"; content: string };

export default function ChatWithHistory() {
  import type { IndexItem } from "../storage/session";
  ...
  async function doExport(id: string) {
    const conv = await loadConversation(id);
    if (!conv) return;
    const txt = await exportConversationAsText(conv.id);
