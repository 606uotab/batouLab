//! Frontend appelle: invoke("chat_complete", { input: { provider, model, api_key?, messages } })
...
pub struct Message {
  pub role: Role,
  pub content: String,
  #[serde(default)] pub ts: Option<i64>,
}
...
// modules providers
mod providers;
use providers::{openai, anthropic, mistral};
...
fn main() {
  ...
  .invoke_handler(tauri::generate_handler![chat_complete]) // <- même nom exact
