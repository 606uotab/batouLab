...
=======
# update.sh — synchro version UI + titre fenêtre à partir de package.json

set -euo pipefail

# 1) Récupérer version dans package.json (source de vérité)
VER="$(grep -oE '"version": *"[0-9]+\.[0-9]+\.[0-9]+"' package.json \
       | head -1 \
       | grep -oE '[0-9]+\.[0-9]+\.[0-9]+')"

if [ -z "$VER" ]; then
  echo "Impossible de lire la version dans package.json"
  exit 1
fi

echo "Version détectée: $VER"

# 2) Mettre à jour titre Tauri (fenêtre native)
CONF="src-tauri/tauri.conf.json"
if [ -f "$CONF" ]; then
  sed -i -E 's/"title": *"batouLab V?[0-9]+\.[0-9]+\.[0-9]+"/"title": "batouLab V'"$VER"'"/' "$CONF"
  echo "✓ $CONF -> title = batouLab V$VER"
fi

# 3) Mettre à jour titre HTML (onglet navigateur intégré)
HTML="index.html"
[ -f "public/index.html" ] && HTML="public/index.html"

if [ -f "$HTML" ]; then
  sed -i -E 's|<title>batouLab( V)?[0-9]+\.[0-9]+\.[0-9]+</title>|<title>batouLab '"$VER"'</title>|' "$HTML"
  echo "✓ $HTML -> <title>batouLab $VER</title>"
fi
